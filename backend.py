...
data = pd.read_csv(path + "datapoints.csv")
...
def home():
    ...
    return render_template('index.html', all_k_values = k_values, kselected = 2, plot_graph = orig_graph, step_size = 1)
...
def reset():
    ...
    click = 0
    orig_graph = plot_original(data)
    return render_template('index.html', all_k_values = k_values, kselected = k, plot_graph = orig_graph, step_size = 1)
...
def predict():
    ...
    if click == 1: #First iteration
        ...
        prev_centroids = [[round(prev_centroids[nr][nc], 1) for nc in range(len(prev_centroids[0]))] for nr in range(len(prev_centroids))]
        all_centroids = recal_centroid(cur_data)
        cur_data = assign_group(cur_data, all_centroids)
        graph = plot_grouped(cur_data)
        all_centroids = [[round(all_centroids[nr][nc], 1) for nc in range(len(all_centroids[0]))] for nr in range(len(all_centroids))]
    ...
    step = 0.1 if click >= 2 else 1 #Round to 1 decimal after the first iteration
    ...
    return render_template('index.html', all_k_values = k_values, kselected = num_k,
                           ...
                           step_size = step) #Preserve changes made by users
